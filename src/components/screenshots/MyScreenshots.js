...
import { deleteScreenShot, getScreenShots, getScreenshotsForArcher } from "../../managers/ScreenShotManager"
...
export const MyScreenshots = ({delScreenShot}) => {
    ...
    const confirmDelete = (screenshot) => {
        ...
        window.confirm(text)
            ? deleteScreenShot(screenshot.id).then(() => {window.location.reload()})
            : <></>
    }
    ...
    return <article className="screenshots">
        <h2 className="screenshotsHeader-title-is-3">My Screenshots</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)' }}>
        {
            screenshotByArchers.map(
            (filteredScreenshot) => {
              
                        return <section key={`screenshots--${filteredScreenshot.id}`} className="screenshot">
                            <div className="screenshot-boxes" id="screenshots">
                                <div className="screenshotDetails-column">
                                    <img src={filteredScreenshot.image} className="screenshot__image" key={`screenshot--${filteredScreenshot.image}`}/>
                                    <div className="archer has-text-left">Content: {filteredScreenshot.content}</div>
                                    <div className="archer has-text-left">Capture Tool: {filteredScreenshot.captureTool}</div>
                                    <div className="archer has-text-left">Editing Tool: {filteredScreenshot.editingTool}</div>

                                    {/* <div className="archer has-text-left">category: {filteredScreenshot.category}</div> */}
                                    <div className="archer has-text-left">Time Stamp: {filteredScreenshot.timestamp}</div>

                                    <div className="edit_screenshot">
                                    {
                                        ScreenshotArcherObject.username === filteredScreenshot.archer.user.username
                                      
                                            ? 
                                            <button className="cardBtn" onClick={() => navigate(`/screenshots/${filteredScreenshot.id}/update`)}>Edit</button>
